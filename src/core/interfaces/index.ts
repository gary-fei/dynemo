...
import {
  ...
  MAP_INNER_REG,
  MAP_REG_GLOBAL,
  CHINESE_AND_CHARACTER_CONVERSION_REG,
  CHINESE_CHAER_REF,
} from '../utils/const'
...
export function convertDefinitionProperty(definitionProperty: string): string {
  const value = pinyin(definitionProperty).replace(/\s*/g, '');
  return CHINESE_CHAER_REF.test(definitionProperty) ?( `customize_${value}`).replace(CHINESE_AND_CHARACTER_CONVERSION_REG,''):definitionProperty.replace(/\W+$/, '').replace(/\W+/g, '_')
}
